...
int Util::CompressString_vec(const std::vector<unsigned char>& src, std::string& dst, int compressionlevel) {
    ...
    auto srcp = static_cast<const void*>(src.data());  // 修改为处理字节数组
...
int Util::DecompressString_vec(const std::vector<unsigned char>& src, std::string& dst) {
    // 将 vector 转换为 string
    //const std::string src(reinterpret_cast<const char*>(src.data()), src1.size());
    size_t const cBuffSize = ZSTD_getFrameContentSize(src.data(), src.size());
    ...
    if (ZSTD_CONTENTSIZE_UNKNOWN == cBuffSize) {
        return StreamDecompressString_vec(src, dst);
    }
    ...
    dst.resize(cBuffSize);
    auto dstp = const_cast<void*>(static_cast<const void*>(dst.c_str()));
    auto srcp = static_cast<const void*>(src.data());
...
int Util::StreamDecompressString_vec(const std::vector<unsigned char>& src, std::string& dst, int compressionlevel) {
    ...
    while ((read = std::min(src.size() - local_pos, toRead))) {
        std::copy(src.begin() + local_pos, src.begin() + local_pos + read, buff_tmp);
        local_pos += read;
